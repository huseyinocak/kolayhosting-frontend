...
export const setupAuthInterceptor = (axiosInstance) => {
    ...
    axiosInstance.interceptors.response.use(
        ...
        async (error) => {
            ...
            if (error.response.status === 401 && !originalRequest._retry && originalRequest.url !== `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000/api/v1'}/auth/refresh-token`) {
