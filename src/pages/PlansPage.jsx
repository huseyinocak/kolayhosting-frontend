...
const PlansPage = () => {
    ...
    const pageTitle = useMemo(() => {
        ...
        const currentCategory = categories?.find(cat => String(cat.id) === selectedCategory);
        const currentProvider = providers?.find(prov => String(prov.id) === selectedProvider);

        if (searchTerm) {
            title = `${searchTerm} ${t('plans_search_results', { defaultValue: 'Arama Sonuçları' })}`;
        }
        if (currentCategory && selectedCategory !== 'all') {
            title = `${currentCategory.name} ${t('plans_category_title_suffix', { defaultValue: 'Hosting Planları' })}`;
        }
        if (currentProvider && selectedProvider !== 'all') {
            title = `${currentProvider.name} ${t('plans_provider_title_suffix', { defaultValue: 'Hosting Planları' })}`;
        }
        return `${title} - KolayHosting`;
    }, [searchTerm, selectedCategory, selectedProvider, categories, providers, t]);

    const pageDescription = useMemo(() => {
        let description = t('plans_page_description', { defaultValue: 'KolayHosting\'de yüzlerce hosting planını karşılaştırın. İhtiyaçlarınıza en uygun web hosting, VPS, dedicated sunucu ve diğer hosting çözümlerini bulun.' });
        const currentCategory = categories?.find(cat => String(cat.id) === selectedCategory);
        const currentProvider = providers?.find(prov => String(prov.id) === selectedProvider);
        ...
        if (searchTerm) {
            description = `${t('plans_search_description_prefix', { defaultValue: 'Arama sonuçları:' })} ${searchTerm}. ${description}`;
        }
        return description;
    }, [searchTerm, selectedCategory, selectedProvider, categories, providers, t]);
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <h1 className="text-4xl font-bold text-center text-gray-900 dark:text-white mb-10">{t('view_all_plans')}</h1>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8 items-center justify-center flex-wrap">
                ...
                <Select value={sortBy} onValueChange={handleSortChange}>
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="price_asc">{t('sort_by_price')} ({t('ascending')})</SelectItem>
                        <SelectItem value="price_desc">{t('sort_by_price')} ({t('descending')})</SelectItem>
                        <SelectItem value="renewal_price_asc">{t('renewal_price')} ({t('ascending')})</SelectItem>
                        <SelectItem value="renewal_price_desc">{t('renewal_price')} ({t('descending')})</SelectItem>
