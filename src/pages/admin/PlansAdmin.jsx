...
const planSchema = z.object({
  ...
  renewal_price: z.preprocess(
    ...
    z
      .number()
      .min(0, { message: "Yenileme fiyatı pozitif bir sayı olmalıdır." })
      .optional()
      .nullable()
  ),
  ...
  affiliate_url: z.preprocess(
    (val) => (typeof val === "string" && val.trim() === "" ? undefined : val),
    z.string().url({ message: "Geçerli bir URL giriniz." }).optional()
  ),
...
const PlansAdmin = () => {
  ...
  useEffect(() => {
    if (isDialogOpen && currentPlan) {
      ...
      reset({
        ...
        currency: currentPlan.currency || "TRY",
        link: currentPlan.link || "",
        affiliate_url: currentPlan.affiliate_url || "",
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const payload = {
        ...
        currency: data.currency, // ✅ eklendi
        link: data.link, // ✅ eklendi
        affiliate_url: data.affiliate_url || null, // ✅ eklendi
      };
      ...
      if (currentPlan) {
        ...
        if (newPlanId) {
          ...
          console.warn(
            "Yeni oluşturulan planın ID'si bulunamadı, özellikler senkronize edilemedi."
          );
        }
        toast({
      ...
      toast({
        ...
        description:
          error.message ||
          (currentPlan
            ? t("failed_to_update_plan")
            : t("failed_to_create_plan")),
        variant: "destructive",
      });
      console.error(
        "Plan işlemi hatası:",
  ...
  const handleFeatureToggle = (feature, checked) => {
    ...
    if (checked) {
      ...
      setValue(
        "features",
        currentFeatures.filter((f) => f.feature_id !== feature.id)
      );
    }
  };
  ...
  const isErrorCombined =
    isErrorPlans || isErrorCategories || isErrorProviders || isErrorFeatures;
  const combinedError =
    plansError || categoriesError || providersError || featuresError;

  if (isLoadingCombined) {
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-10">
          {t("admin_plans_page_title")}
        </h1>
  ...
  if (isErrorCombined) {
    return (
      <div className="container mx-auto px-4 py-8 text-center text-red-500">
        <p>
          {t("error_loading_plans")}: {combinedError.message}
        </p>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-4">
      ...
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {currentPlan ? t("edit_plan") : t("create_plan")}
              </DialogTitle>
              <DialogDescription>
                {currentPlan
                  ? t("edit_plan_description")
                  : t("add_new_plan_description")}
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="link">Plan Linki</Label>
                  <Input
                    id="link"
                    type="url"
                    placeholder="https://..."
          ...
          <TableBody>
            {plans.map((plan) => (
              <TableRow key={plan.id}>
                ...
                <TableCell>
                  {providers.find((p) => p.id === plan.provider_id)?.name ||
                    t("unknown")}
                </TableCell>
                <TableCell>
                  {categories.find((c) => c.id === plan.category_id)?.name ||
                    t("unknown")}
                </TableCell>
                <TableCell>
                  {plan.status === "active"
      ...
      <BulkImportDialog
        ...
        fields={[
          ...
          { key: "provider_id", label: "Sağlayıcı ID", required: true },
          { key: "category_id", label: "Kategori ID", required: true },
          { key: "storage_gb", label: "Depolama (GB)" },
          { key: "bandwidth_tb", label: "Trafik (TB)" },
        ]}
        onImported={() => refetch?.()}
      />
    </div>
