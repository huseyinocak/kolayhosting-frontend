...
import { getAllProviders, createProvider, updateProvider, deleteProvider } from '../../api/providers'; // Sağlayıcı API fonksiyonlarını içe aktar
...
const providerSchema = z.object({
    ...
    website_url: z.url({ message: "Geçerli bir URL giriniz." }).optional().or(z.literal('')), // Boş stringi de kabul et
...
const ProvidersAdmin = () => {
    ...
    const { data, isLoading, isError, error } = useQuery({
        ...
        queryFn: () => getAllProviders({ name: search, sort_by: sortBy, sort_order: sortOrder, page, per_page: perPage }),
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            toast({
                ...
                description: error.message || (currentProvider ? t("failed_to_update_provider") : t("failed_to_create_provider")),
                variant: "destructive",
            });
            console.error("Sağlayıcı işlemi hatası:", error.response?.data || error.message);
        }
    };
    ...
    if (isLoading) {
        return (
            <div className="container mx-auto px-4 py-8">
                <h1 className="text-4xl font-bold text-center mb-10">{t('admin_providers_page_title')}</h1>
    ...
    if (isError) {
        return (
            <div className="container mx-auto px-4 py-8 text-center text-red-500">
                <p>{t('error_loading_providers')}: {error.message}</p>
            </div>
        );
    }
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <h1 className="text-4xl font-bold text-center text-gray-900 dark:text-white mb-10">{t('admin_providers_page_title')}</h1>
            ...
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    ...
                    <DialogContent className="sm:max-w-[425px] max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            <DialogTitle>{currentProvider ? t('edit_provider_title') : t('create_provider')}</DialogTitle>
                            <DialogDescription>
                                {currentProvider ? t('edit_provider_description') : t('add_new_provider_description')}
                            </DialogDescription>
                        </DialogHeader>
                        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input id="website_url" type="url" {...register("website_url")} placeholder="https://www.example.com" />
                                {errors.website_url && <p className="text-red-500 text-sm">{errors.website_url.message}</p>}
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="affiliate_url">{t('affiliate_url')}</Label>
                                <Input id="affiliate_url" type="url" {...register("affiliate_url")} placeholder="https://affiliate.example.com" />
                                {errors.affiliate_url && <p className="text-red-500 text-sm">{errors.affiliate_url.message}</p>}
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="logo">{t('logo')}</Label>
                                <Input id="logo" type="file" onChange={handleLogoChange} accept=".jpg,.jpeg,.png,.webp" />
                                {logoPreview && (
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-500 dark:text-gray-400">{t('logo_preview')}</p>
                                        <img src={logoPreview} alt="Logo Preview" className="mt-1 h-20 w-20 object-contain rounded-md border" />
                                    </div>
                                )}
                                {!logoPreview && currentProvider?.logo_url && (
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-500 dark:text-gray-400">{t('current_logo')}</p>
                                        <img src={currentProvider.logo_url} alt="Current Logo" className="mt-1 h-20 w-20 object-contain rounded-md border" />
                                    </div>
                                )}
                                {!logoPreview && !currentProvider?.logo_url && (
                                     <p className="text-sm text-gray-500 dark:text-gray-400">{t('no_logo')}</p>
                                )}
                            </div>
                            <DialogFooter>
                                <Button type="submit" disabled={isSubmitting}>
                                    {isSubmitting ? (currentProvider ? t('updating') : t('creating')) : (currentProvider ? t('save') : t('create'))}
                                </Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <Select onValueChange={(value) => { setSortBy(value); setPage(1); }} value={sortBy}>
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value="average_rating">{t('average_rating')}</SelectItem>
                    ...
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="cursor-pointer hover:text-gray-700 dark:hover:text-gray-300" onClick={() => handleSort('name')}>
                                {t('provider_name')} {sortBy === 'name' && <ArrowUpDown className={`inline-block ml-1 h-4 w-4 ${sortOrder === 'asc' ? 'rotate-180' : ''}`} />}
                            </TableHead>
                            <TableHead>{t('provider_website')}</TableHead>
                            <TableHead className="cursor-pointer hover:text-gray-700 dark:hover:text-gray-300" onClick={() => handleSort('average_rating')}>
                                {t('provider_rating')} {sortBy === 'average_rating' && <ArrowUpDown className={`inline-block ml-1 h-4 w-4 ${sortOrder === 'asc' ? 'rotate-180' : ''}`} />}
                            </TableHead>
                            <TableHead className="cursor-pointer hover:text-gray-700 dark:hover:text-gray-300" onClick={() => handleSort('created_at')}>
                                {t('provider_created_at')} {sortBy === 'created_at' && <ArrowUpDown className={`inline-block ml-1 h-4 w-4 ${sortOrder === 'asc' ? 'rotate-180' : ''}`} />}
                            </TableHead>
                            <TableHead className="text-right">{t('provider_actions')}</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                            ...
                            <TableRow key={provider.id}>
                                ...
                                <TableCell>
                                    <Avatar className="h-10 w-10">
                                        <AvatarImage src={provider.logo_url || `https://placehold.co/40x40/aabbcc/ffffff?text=${provider.name.charAt(0)}`} alt={provider.name} />
                                        <AvatarFallback>{provider.name.charAt(0)}</AvatarFallback>
                                    </Avatar>
                                </TableCell>
                                <TableCell>{provider.name}</TableCell>
                                <TableCell>
                                    <a href={provider.website_url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                                        {provider.website_url ? new URL(provider.website_url).hostname : 'N/A'}
                                    </a>
                                </TableCell>
                                <TableCell>{!isNaN(Number(provider.average_rating))
                                        ? Number(provider.average_rating).toFixed(1)
                                        : 'N/A'
                                    } ({provider.reviews_count || 0})</TableCell>
                                <TableCell>{new Date(provider.created_at).toLocaleDateString('tr-TR')}</TableCell>
                                <TableCell className="text-right flex space-x-2 justify-end">
                                    <Button variant="outline" size="sm" onClick={() => handleEditProviderClick(provider)}>
