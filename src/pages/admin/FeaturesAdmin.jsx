...
const featureSchema = z.object({
  name: z
    .string()
    .min(2, { message: "Özellik adı en az 2 karakter olmalıdır." }),
    ...
    try {
      ...
      toast({
        ...
        description:
          error.message ||
          (currentFeature
            ? t("failed_to_update_feature")
            : t("failed_to_create_feature")),
        variant: "destructive",
      });
      console.error(
        "Özellik işlemi hatası:",
  ...
  if (isError) {
    return (
      <div className="container mx-auto px-4 py-8 text-center text-red-500">
        <p>
          {t("error_loading_features")}: {error.message}
        </p>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-4">
      ...
      <div className="flex justify-between items-center mb-6 flex-wrap gap-2">
        <div className="flex items-center space-x-2 flex-wrap gap-1">
          <Input
            ...
            className="w-80 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
          />
          <Select
            onValueChange={(value) => {
              setSortBy(value);
          ...
          <TableBody>
            {features.map((feature) => (
              <TableRow key={feature.id}>
                ...
                <TableCell>
                  {new Date(feature.created_at).toLocaleDateString("tr-TR")}
                </TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>
              {currentFeature ? t("edit_feature_title") : t("create_feature")}
            </DialogTitle>
            <DialogDescription>
              {currentFeature
                ? t("edit_feature_description")
                : t("add_new_feature_description")}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                id="unit"
                {...register("unit")}
                placeholder={t("feature_unit_placeholder")}
              />
              {errors.unit && (
      ...
      <BulkImportDialog
        ...
        fields={[
          ...
          { key: "type", label: "Tür" }, // örn: text | number | boolean (projeye göre)
          { key: "unit", label: "Birim" }, // örn: GB, TB, adet...
        ]}
        onImported={() => refetch?.()}
      />
    </div>
